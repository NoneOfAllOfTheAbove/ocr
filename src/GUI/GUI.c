...
/*------------------File's GCallback------------------*/
static void loadImage_activated (
	GtkWidget *fileMenu_loadImage, gpointer window
)
{

	/*We have to put the first parameter even if we are not using.
	This is to respect convention. If we don't put it,
	it will show a warning explaining that we are trying
...
static void exportText_activated(
...
static void helpAbout_activated()
{
	g_app_info_launch_default_for_uri(
		"https://github.com/NoneOfAllOfTheAbove/OCR", NULL, NULL
	);
}
...
{   
	...
	if(image != NULL)
	{
		...
		/*pixbuf = gdk_pixbuf_new_from_file_at_size(
			"resources/lastDebugScreenshot.png",
			boxA_width, boxA_height,
			NULL
		);*/
...
//For me, keep int main(). But when have to push,
// replace int main() -> int StartGUI().
int StartGUI(int argc, char *argv[])
{
	...
	*/
	...
	gtk_menu_shell_append(
		GTK_MENU_SHELL(fileMenu_exportMenu), fileMenu_exportMenu_text
	);

	/*Add "fileMenu_exportMenu" to "fileMenu_export"*/
	gtk_menu_item_set_submenu(
		GTK_MENU_ITEM(fileMenu_export), fileMenu_exportMenu
	);

	/*Add "fileMenu_loadImage/export/quit" to fileMenu*/
	...
	gtk_menu_shell_append(
		GTK_MENU_SHELL(fileMenu), gtk_separator_menu_item_new()
	);
	...
	//For Loading Image
	g_signal_connect(
		G_OBJECT(fileMenu_loadImage), "activate",
		G_CALLBACK(loadImage_activated), window
	);
	//For Export -> Test
	g_signal_connect(
		G_OBJECT(fileMenu_exportMenu_text), "activate",
		G_CALLBACK(exportText_activated), window
	);
	//For Quit
	g_signal_connect(
		G_OBJECT(fileMenu_quit), "activate",
		G_CALLBACK(quit_activated), NULL
	);
	...
	optionMenu_debugMode =
		gtk_check_menu_item_new_with_label("Enable debug mode");
	gtk_check_menu_item_set_active(
		GTK_CHECK_MENU_ITEM(optionMenu_debugMode),
		FALSE
	);
	gtk_menu_shell_append(GTK_MENU_SHELL(optionMenu), optionMenu_debugMode);
	g_signal_connect(
		G_OBJECT(optionMenu_debugMode), "activate",
		G_CALLBACK(debugMode_activated), NULL
	);
	...
	g_signal_connect(
		G_OBJECT(optionMenu_filters), "activate",
		G_CALLBACK(filters_activated), NULL
	);
	...
	gtk_menu_shell_append(
		GTK_MENU_SHELL(optionMenu), optionMenu_postprocessing
	);
	g_signal_connect(
		G_OBJECT(optionMenu_postprocessing), "activate",
		G_CALLBACK(postprocessing_activated), NULL
	);
	...
	//For About.
	g_signal_connect(
		G_OBJECT(helpMenu_about), "activate",
		G_CALLBACK(helpAbout_activated), NULL
	);
	...
	//For Run button
	g_signal_connect(
		G_OBJECT(extractTextButton), "clicked",
		G_CALLBACK(extractText_activated), NULL
	);
	...
	*/
	...
	/*-------------------------------CASE 2------------------------------*/
	//Add "boxA" and "boxB" to "box2".
	//"box2" will later be add to case(2) of "mainBox"
